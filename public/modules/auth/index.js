...
function run($window, $rootScope, $state, jwtHelper, signinModal, HAS_MODAL_LOGIN, UserTokenStorage, Auth) {
  ...
  $rootScope.$on('auth-is-authenticated', function(event, data) { 
    ...
    console.log($rootScope.global.isAdmin());
  });
  ...
  $rootScope.global.isAdmin = function() {
    if(!$rootScope.global.isAuthenticated){
      return false;
    }
    console.log($rootScope.global.isAuthenticated.scope,$rootScope.global.isAuthenticated.scope.indexOf('admin'));
    return $rootScope.global.isAuthenticated.scope.indexOf('admin') !== -1;
